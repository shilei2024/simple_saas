-- Harden credit reservation RPC:
-- - Prevent accidental PUBLIC execution of a SECURITY DEFINER function
-- - Fix true idempotency under concurrent calls (unique_violation retry)
-- - Lock down search_path to avoid injection via SECURITY DEFINER
...
declare
  ...
  v_lot record;
begin
  -- Defense in depth: only allow service role to run this SECURITY DEFINER function.
  ...
  exception
    when unique_violation then
      -- Another concurrent transaction inserted credit_usage first. Return the existing row.
      select cu.kind, cu.lot_id
        into v_existing
      from public.credit_usage cu
      where cu.email_correspondence_id = p_email_correspondence_id
...
-- Lock down EXECUTE privileges: SECURITY DEFINER must never be callable by PUBLIC.
revoke all on function public.reserve_correspondence_credit_usage(uuid, uuid, text, uuid, jsonb) from public;
revoke all on function public.reserve_correspondence_credit_usage(uuid, uuid, text, uuid, jsonb) from anon;
revoke all on function public.reserve_correspondence_credit_usage(uuid, uuid, text, uuid, jsonb) from authenticated;
